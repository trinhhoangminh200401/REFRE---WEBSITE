  ...
  //           let allCondition11 = answerschoice[answerschoice.length - 1].some(
  //                 item => item === "Hương cây cỏ thực vật" || item === "Hương thơm thực vật " || item==="Hương hoa trái cây"
  //               ) && answerschoice[answerschoice.length - 1].includes ("Hương thơm hoa nhẹ nhàng tinh tế")
  //             //   || item === "Hướng nội sâu lắng" || item === "Hẹn hò, đi chơi" ||
  //             //  item === "Vintage nhẹ nhàng" || item ==="Tưới mới lạc quan"



  //           let allCondition12 = answerschoice[answerschoice.length - 1].some(
  //                 item => item === "Hương cây cỏ thực vật" || item === "Hương thơm thực vật " || item==="Hương hoa trái cây"
  //               ) && answerschoice[answerschoice.length - 1].includes("Hương thơm xạ hương")
  //           let allCondition13 = answerschoice[answerschoice.length - 1].some(
  //                 item => item === "Hương cây cỏ thực vật" || item === "Hương thơm thực vật" || item==="Hương hoa trái cây"
  //           ) && answerschoice[answerschoice.length - 1].includes("Hương gỗ, mùi đất")
  //           let allCondition14 = answerschoice[answerschoice.length - 1].some(
  //             item => item === "Hương cây cỏ thực vật" || item === "Hương thơm thực vật" || item==="Hương hoa trái cây"
  //           ) && answerschoice[answerschoice.length - 1].includes("Hương vani, hổ phách")
  //           let allCondition15= answerschoice[answerschoice.length - 1].some(
  //             item => item === "Hương cây cỏ thực vật" || item === "Hương thơm thực vật " || item==="Hương hoa trái cây"
  //           ) && answerschoice[answerschoice.length - 1].includes("Hương kẹo ngọt")
  //           let allCondition16 =answerschoice[answerschoice.length - 1].some(
  //             item => item === "Hương cây cỏ thực vật" || item === "Hương thơm thực vật " || item==="Hương hoa trái cây"
  //           ) && answerschoice[answerschoice.length - 1].includes("Hương hoa thơm nồng")
  //           let allCondition17 = answerschoice[answerschoice.length - 1].some(
  //             item => item === "Hương thơm hoa nhẹ nhàng tinh tế" || item === "Hương gỗ, mùi đất" || item==="Hương vani, hổ phách"
  //           ) && answerschoice[answerschoice.length - 1].includes("Hương kẹo ngọt")
  //           let allCondition18 = answerschoice[answerschoice.length - 1].some(
  //             item => item === "Hương thơm hoa nhẹ nhàng tinh tế" || item === "Hương gỗ, mùi đất" || item==="Hương vani, hổ phách"
  //           ) && answerschoice[answerschoice.length - 1].includes("Hương thơm xạ hương")
  //           let allCondition19 = answerschoice[answerschoice.length - 1].some(
  //             item => item === "Hương thơm hoa nhẹ nhàng tinh tế" || item === "Hương gỗ, mùi đất" || item==="Hương vani, hổ phách"
  //           ) && answerschoice[answerschoice.length - 1].includes("Hương thơm thực vật ")
  //           let allCondition20=answerschoice[answerschoice.length - 1].some(
  //             item => item === "Hương thơm hoa nhẹ nhàng tinh tế" || item === "Hương gỗ, mùi đất" || item==="Hương vani, hổ phách"
  //           ) && answerschoice[answerschoice.length - 1].includes("Hương cây cỏ thực vật")
  ...
  //           console.log("thg 11", allCondition11)
  //           console.log("thg 12", allCondition12)
  //           console.log("thg 13", allCondition13)
  //           console.log("Thg 14", allCondition14)
  //           console.log("thg 15", allCondition15)
  //           console.log("thg 16", allCondition16)
  //           console.log("thg 17", allCondition17)
  //           console.log("thg 18", allCondition18)
  //           console.log("thg 19", allCondition19)
  //           console.log("thg 20", allCondition20)
  //           if (allConditionsMet)
  //           {
  ...
  //                 </div>`);
  //           }
  //           else if (allCondition20 && allCondition11)
  //           {
  //              returnResult.html(`<div class ="headline my-3">
  //                   <img src="/assets/images/Quiz/Trang Trac Nghiem/Answer_GreenTea_Headline.png" />
  //                 </div>
  ...
  class Quiz
  {
   ...
   loadQuestionnaireState() {
    ...
    handleResponse (index, selectedItem)
    {
      ...
      let updatedQuestionnaire = this.cloneArray(newQuestionnaire);
    ...
    HandleOnchange (e)
    {
      const $checkbox = $(e.target).closest('.image-checkbox').find('input[type="checkbox"]');
      const name = $checkbox.attr("name");
      if (name)
      {
        ...
        {
          // If the checkbox is being checked and the maxSelection limit is reached, prevent further selection
          $(e.target).removeClass("image-checkbox-checked");
          $checkbox.prop("checked", false);
          unselectedCheckboxes.prop("disabled", true);
        } else if (
          !$checkbox.prop("checked") &&
        ...
        {
          // Otherwise, toggle the checked state and handle the response
          $(e.target).toggleClass("image-checkbox-checked", $checkbox.prop("checked"));
          unselectedCheckboxes.prop("disabled", false);

          this.handleResponse(index, selectedItem);
          this.saveQuestionnaireState();
          this.loadQuestionnaireState();
        }
      } else
      {
        console.error("Name attribute not found for checkbox:", $checkbox);
      }
    }
  ...
  `)
    ...
    initialize ()
    {
      ...
      this.renderData()
      this.saveQuestionnaireState()
      $(".image-checkbox").on("change", this.HandleOnchange.bind(this))
      $("#handlesubmit").on("click", this.handleSubmit.bind(this));
    }
